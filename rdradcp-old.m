...
switch cfg.sourceprog,
  case 'WINRIVER',
    adcp=struct('name','adcp','config',cfg,'mtime',zeros(1,n),'number',zeros(1,n),'pitch',zeros(1,n),...
        	...
        	'error_vel',zeros(cfg.n_cells,n),'corr',zeros(cfg.n_cells,4,n),'perc_good',zeros(cfg.n_cells,4,n),...
...
clear global ens
% Loop for all records/ensembles
for k=1:n,

  % Gives display so you know something is going on...
    
  if rem(k,1000)==0 | k==1,  fprintf('\n%d',k*num_av); end
  if rem(k,50)==0, fprintf('.'); end

  % Read an ensemble
  
  ens=rd_buffer(fd,num_av);

  if ~isstruct(ens), % Aborting when EOF found...
    fprintf('\n');
