...
%  [ADCP,CFG]=RDRADCP(NAME) reads the binary RDI Workhorse file NAME and
...
%  [..]=RDRADCP(NAME,NUMAV,NENS) reads only NENS ensembles.
...
%
%  ALSO - if you get a file that doesn't work (in particular if you get a 'not a workhorse
%         file or bad data' message; the FIRST thing to try is to scroll
%         down to around line 200 of this file and read the comments in the 
%         asterisked box.
%
...
century=2000;  % ADCP clock does not have century.
...
nens=nens+1;  % RD added this jem as it wasn't reading the last ensemble
...
adcp=struct('name','wh-adcp','mtime',zeros(1,n),'number',zeros(1,n),'pitch',zeros(1,n),...
            ...
            'error_vel',zeros(cfg.n_cells,n),'intens',zeros(cfg.n_cells,4,n),'bt_range',...
	    zeros(4,n),'bt_vel',zeros(4,n));
...
%******************************************************************************
% Some weirdness about total record lengths (see notes in rd_buffer below)
% The RDI manual suggests that hdr.nbytes is the total number of bytes in a record,
% plus 2 for a checksum. However, I have found cases where I need to add an *extra*
% 2 as well. I do not know yet *why* this would be the case - limited information
% suggests it *may* have sometbing to do with whether the number of bytes is odd
% or even (then again, it may not). So if things don't work, try forcing
% this extraoffset to 0 or 2.	  
if rem(hdr.nbyte,2)==0, hdr.nbyte=hdr.nbyte+2; end;
%******************************************************************************
...
if num_av<0 | isempty(ens),
 ...
 ens=struct('number',zeros(1,n),'rtc',zeros(7,n),'BIT',zeros(1,n),'ssp',zeros(1,n),'depth',zeros(1,n),'pitch',zeros(1,n),...
            ...
            'corr',zeros(cfg.n_cells,4,n),'bt_range',zeros(4,n),'bt_vel',zeros(4,n));
  num_av=abs(num_av);
end;
...
while k<num_av,
   ...
   % # bytes per record varies mysterously, so instead I will
   % search forward for the next Header ID rather than trying to
   % Count every byte.
...
%   while ~strcmp(id1,'7F7F')
%      id1=dec2hex(fread(fd,1,'uint16'));
%      ll=ll+1;
%   end;
%%   id2=dec2hex(fread(fd,1,'uint8'));
%%   if ~strcmp(id2,'7F') error(['Not a workhorse file ->' id1 id2]); end;
...
%%    disp(dec2hex(id,4));
    ...
    % reading at the next valid offset.
  ...
  % I used to follow this with a clause that would attempt to
  % get the correct number of bytes per record but this number
  % varied mysteriously by +/- 2 bytes so instead I am trying
  % something a bit more robust (?). Maybe this has something to do with
  % an odd or even # bytes in the record?
  readbytes=ftell(fd)-startpos;
  tb=0; % Dewey had to change this from 2 to 0 for firmware VENUS 16.28 UGH!
  %fprintf('Adjust location by %d (readbytes=%d, hdr.nbyte=%d)\n',hdr.nbyte+tb-byte_offset,readbytes,hdr.nbyte);
  fseek(fd,hdr.nbyte+tb-byte_offset,'cof'); % This 2 is for the checksum
