function [adcp]=rdradcpjmk(name,varargin);
% RDRADCP  Read (raw binary) RDI ADCP files,
%  [ADCP]=RDRADCPjmk(NAME) reads the raw binary RDI BB/Workhorse ADCP file NAME and
...
%
%  Jody Klymak Modified to read all data and parse from RAM.
...
ch = uint8(fread(fd,Inf,'uint8'));  % read the entire file into RAM
nbytes=length(ch);
disp([num2str(nbytes/(1024*1024)),' MB of data read.']);
...
% This is handled by the above code that assumes the maximum ensemble byte size bpens
...
while i<length(in0), % Identify the header blocks
  if (in0(i+1)-in0(i)) == (bpens+2), % then ino(i) is the start of a header block
    j=j+1;
    in(j)=in0(i);
...
indx=find(~isnan(in));
in=in(indx);   % this is the estimate of the number of ensembles
...
adcp.range = ldr.bin1+((1:ldr.nbins)-1)*ldr.binl;

% preallocate memory and set initially to NaNs
adcp.cor = zeros(ldr.nbins,length(in),4)+NaN;
adcp.int = zeros(ldr.nbins,length(in),4)+NaN;
...
for i=1:length(in);
 ...
 % if i>515, format long, datestr(lasttime), [i length(dat) hdr.offsets' ids'], end
 % ldr = getleader(dat,hdr.offsets(1));
  varl = getvar(dat,hdr.offsets(2),varl,i);
  lasttime=varl.time(i);
  offsets = hdr.offsets;
  nbins = ldr.nbins;
  for j=3:length(hdr.offsets)
    switch ids(j)
      ...
      case 8192
        ...
        otherwise % unknown/unwanted/corrupt data type/pointer
        [num2str(j),' ',dec2hex(ids(j))]
        %return
    end   
...
in=find(adcp.cor==-32768);adcp.cor(in)=NaN;
in=find(adcp.int==-32768);adcp.int(in)=NaN;
