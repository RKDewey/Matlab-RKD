function [pspec,k,f]=ps2d(x,dx,dt,mfft,nfft,avg)
%  [pspec,k,f]=ps2d(x,dx,dt,mfft,nfft) calculates 2D power spectra
...
%     mfft,nfft: length of fft segments, =1 to guestimate, 
...
if exist('mfft'),  % check to see if user has set fft length
   M=mfft;         % yes...
else
   M=1;            % no... so do next if loop
end
if M == 1,
   m2=2.^(4:13);   % maximum auto spectral length is 8192
   M=m2(max(find(m2<fix(m/5)))); % choose the longest M with at least 5 segments
   if isempty(M),  % if that didn't work, choose less segments
      M=m2(max(find(m2<fix(m)))); % choose the longest M with 1 segments
...
if exist('nfft'),  % check to see if user has set fft length
   ...
   N=1;            % no... so do next if loop
end
if N == 1,
   n2=2.^(4:13);   % maximum auto spectral length is 8192
   N=n2(max(find(n2<fix(n/5)))); % choose the longest M with at least 5 segments
   if isempty(N),  % if that didn't work, choose less segments
      N=n2(max(find(n2<fix(n)))); % choose the longest M with 1 segments
...
windowm=hanning(M);
Im=(sum(windowm.^2));
Wm=M/Im;               % variance lost by windowing
windown=hanning(N)';
In=(sum(windown.^2));
Wn=N/In;               % variance lost by windowing
%
