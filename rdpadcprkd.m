function [adcp,cfg,ens]=rdpadcprkd(varargin);
% ADCP=RDPADCPrkd(NAME) reads the p-file NAME and returns a (self-explanatory)
...
%  P-files can be created by TRANSECT or WINRIVER.
%
...
if strcmp(cfg.adcp_type,'broadband'), % BB P-file format
  ...
  cfg.ssp            =fread(fd,1,'uint32')*.01;    % m/s
...
%  fprintf('ID: %s\n',dec2hex(id,4));
    ...
    case '000C'   % Discharge
       % 18/04/2002 - received the following explanation for discharge format from RDI:
       %
       % NOTE:  The last bin of Discharge Data is reserved and contains only 2 bytes
       % of data.  This is not valid discharge information.  All other bins are
       % correct and valid.  For example, if 30 bins were collected then only bins 
       % 1-29 will have valid discharge data of 4 bytes per bin.  Bin 30 (the last 
       % bin in this example) is reserved for RDI use and contains only 2 bytes of
       % information.
       %
       % Note - NB P-file didn't seem to have this problem, but in general I don't gather
       % or store discharge data so this is really untested code.
       %
      if strcmp(cfg.adcp_type,'narrowband'),
        fseek(fd,4*hdr.n_deps,'cof'); % Seems to be neccessary on Tully 28/june/00
%%	dec2hex(fread(fd,10,'uint16'),4)
      else                             
        fseek(fd,4*hdr.n_deps,'cof');
%%	dec2hex(fread(fd,hdr.n_deps,'uint32'),4)
      end;
