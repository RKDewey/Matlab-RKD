...
if ~isstruct(t_input)
    tt = datevec(t_input);
    time.UTC=0*ones(length(t_input),1);
    time.year = tt(:,1);
    time.month = tt(:,2);
    time.day = tt(:,3);
    time.hour = tt(:,4);
...
X0 = p(1) * JCE.^3 + p(2) * JCE.^2 + p(3) .* JCE + p(4)*ones(size(JCE)); % This is faster than polyval...

% 2. Mean anomaly of the sun (earth)
p = [-(1/300000) -0.0001603 35999.05034 357.52772];
% X1 = polyval(p, JCE);
X1 = p(1) * JCE.^3 + p(2) * JCE.^2 + p(3) .* JCE + p(4)*ones(size(JCE)); 

% 3. Mean anomaly of the moon
p = [(1/56250) 0.0086972 477198.867398 134.96298];
% X2 = polyval(p, JCE);
X2 = p(1) * JCE.^3 + p(2) * JCE.^2 + p(3) .* JCE + p(4)*ones(size(JCE)); 

% 4. Moon argument of latitude
p = [(1/327270) -0.0036825 483202.017538 93.27191];
% X3 = polyval(p, JCE);
X3 = p(1) * JCE.^3 + p(2) * JCE.^2 + p(3) .* JCE + p(4)*ones(size(JCE)); 
...
% X4 = polyval(p, JCE);
X4 = p(1) * JCE.^3 + p(2) * JCE.^2 + p(3) .* JCE + p(4)*ones(size(JCE)); 
...
mean_obliquity = p(1)*U.^10 + p(2)*U.^9 + p(3)*U.^8 + p(4)*U.^7 + p(5)*U.^6 + p(6)*U.^5 + p(7)*U.^4 + p(8)*U.^3 + p(9)*U.^2 + p(10)*U + p(11)*ones(size(U));

for i=1:length(U),
true_obliquity(i) = (mean_obliquity(i)/3600) + nutation.obliquity(i);
end
...
for i=1:length(aberration_correction),
apparent_sun_longitude(i) = sun_geocentric_position.longitude(i) + nutation.longitude(i) + aberration_correction(i);
end
...
for i=1:length(true_obliquity),
argument_numerator(i) = (sin(apparent_sun_longitude(i) * pi/180) .* cos(true_obliquity(i) * pi/180)) - ...
    (tan(sun_geocentric_position.latitude(i) * pi/180) .* sin(true_obliquity(i) * pi/180));
argument_denominator = cos(apparent_sun_longitude(i) * pi/180);
end
...
for i=1:length(true_obliquity),
argument(i) = (sin(sun_geocentric_position.latitude(i) * pi/180) * cos(true_obliquity(i) * pi/180)) + ...
    (cos(sun_geocentric_position.latitude(i) * pi/180) * sin(true_obliquity(i) * pi/180) * sin(apparent_sun_longitude(i) * pi/180));
end
...
for i=1:length(sun_rigth_ascension),
observer_local_hour(i) = apparent_stime_at_greenwich(i) + location.longitude - sun_rigth_ascension(i);
end
