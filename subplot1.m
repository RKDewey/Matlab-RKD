function theAxis = subplot1(nrows, ncols, thisPlot, lsp, rsp, bsp, tsp)
%SUBPLOT1 Create axes in tiled positions.
% 	SUBPLOT1(m,n,p,lsp,rsp,bsp,tsp) breaks the Figure window into
% 	an m-by-n matrix of small axes, selects the p-th axes for 
% 	for the current plot, and returns the axis handle. Adds extra 
%	normalized space (i.e. >1 increases, < 1 decreases) to
%       the left, right, bottom, top (lsp,rsp,bsp,tsp) of each subplot
%       The axes are counted along the top row of the Figure window, then the
%	second row, etc.
...
elseif narg >= 3
	% passed in subplot(m,n,p,lsp,rsp) -- we should kill overlaps
	% here too:
	kill_siblings = 1;
end
...
elseif(isempty(position))
	if (thisPlot < 1)
		...
		PERC_OFFSET_L = lsp*0.09;
		PERC_OFFSET_R = rsp*0.045;
		PERC_OFFSET_B = bsp*0.09;
		PERC_OFFSET_T = tsp*0.045;
		if nrows > 2
...
% For this to work the default axes position must be in normalized coordinates
		...
		totalheight = def_pos(4) - PERC_OFFSET_T;
