function earth = calc_uvw2(BEAM,HH,PP,RR,EL,AZ,GOOD,OPTS,C,WF,WN,WS,UD,TY)
...
%  eg. earth = CALC_uvw2(BEAM,HH,PP,RR,[],[],GOOD,[],[],[],[],WS,UD,TY)
%
% parameters:
...
P   = f * PP;
r   = f * RR;
R   = f * (RR + 180);  % Used in rotation to earth frame if upward config.
el  = f * EL;
az = f * AZ;
[i,j] = size(C);
if i == 1
...
% ----- calculate Beam Directional Matrix from passed azimuth and elevation --
        BeamCalc = [ -cos(el(1))*sin(az(1)) -cos(el(1))*cos(az(1))  ...
                                 -sin(el(1))            0 ;
                     -cos(el(2))*sin(az(2)) -cos(el(2))*cos(az(2))  ...
                                 -sin(el(2))            0 ;
                     -cos(el(3))*sin(az(3)) -cos(el(3))*cos(az(3))  ...
                                 -sin(el(3))            0 ;
                     -cos(el(4))*sin(az(4)) -cos(el(4))*cos(az(4))  ...
                                 -sin(el(4))            0];                             
...
for j = 1:SIZ(1)
	...
	C2 = C(j,2);
        ...
        Mr = [cos(r2) 0 sin(r2);0 1 0;-sin(r2) 0 cos(r2)];
        if UD == 1  %if up, must roll extra 180.  MATRIX calculated for downward.
          Mtot = Mh * Mp * MR;
        else
          Mtot = Mh * Mp * Mr;
        end
	...
	if OPTS(2) == 1						% RDI code
 		...
 		SC  = sin(abs(el))*cos(P2)*cos(r2) + cos(el) .* ...
	...
	for k = 1:SIZ(2)				% each nominal bin
		...
		if calc == 0
			...
			tempbeam( find(flagg2==1) ) = ...
				-sum(temp(find(flagg2==0))) ...
				/ MATRIX(4,find(flagg2==1));
			calc = 1;
		    end
		    if calc == 1
